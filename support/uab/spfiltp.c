static char rcsid[] = "$Author: djh $ $Date: 1991/07/01 06:42:11 $";
static char rcsident[] = "$Header: /mac/src/cap60/support/uab/RCS/spfiltp.c,v 2.1 1991/07/01 06:42:11 djh Rel djh $";
...
struct ifreq *ifr;
{
  ...
  if ((s = pfopen(ifr->ifr_name, O_RDWR)) < 0) {
...
u_char *ea;
{
  ...
  eph = &ephlist[edx-1];
  if (ioctl(eph->socket, EIOCDEVP, &endev) < 0) {
